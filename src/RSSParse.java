import java.io.*;
import java.net.*;

public class RSSParse {
	private Valinta valinta;
	private URL lahde;
	private String[] otsikot;
	private final int count = 10;
	public RSSParse(Valinta x) {
		valinta = x;
	}
	
	public void parse() {
		String inputLine = "";
		otsikot = new String[count];
		int apu = 0;
		try {
			RSSUrl rss = new RSSUrl(valinta);
			lahde = rss.returnURL();
		}
		catch (MalformedURLException mue)	{ mue.printStackTrace(); }
		try {
			BufferedReader in = new BufferedReader(new InputStreamReader(lahde.openStream()));
			while ((inputLine = in.readLine()) != null && apu < count) {
				if (inputLine.contains("<title><![CDATA[") && !inputLine.contains("HS.fi]")) {
					inputLine = inputLine.trim();
					inputLine = inputLine.substring(16, inputLine.length() - 11);
					otsikot[apu] = inputLine;
					apu++;
				}					
			}
			in.close();
		}
		catch (IOException ioe) { ioe.printStackTrace(); }
	}
	
	public void print() {
		for (int i = 0; i < count; i++)
			if (otsikot[i] != null)
				System.out.println(otsikot[i]);
	}
}